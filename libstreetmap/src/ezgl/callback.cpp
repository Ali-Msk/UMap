...
gboolean scroll_mouse(GtkWidget *, GdkEvent *event, gpointer data)
{
  ...
  if(event->type == GDK_SCROLL) {
    ...
    //making sure user never zooms out past map bounds
    if(canvas->get_camera().get_world().area()>canvas->get_camera().get_initial_world().area()){
...
gboolean press_up(GtkWidget *, gpointer data)
{
  ...
  //making sure user never zooms out past map bounds
  if(canvas->get_camera().get_world().top()+5>canvas->get_camera().get_initial_world().top()){
      return TRUE;
  }
...
gboolean press_down(GtkWidget *, gpointer data)
{
  ...
  auto canvas = application->get_canvas(main_canvas_id);
      ...
      //making sure user never zooms out past map bounds
  if(canvas->get_camera().get_world().bottom()-5<canvas->get_camera().get_initial_world().bottom()){
      return TRUE;
  }
...
gboolean press_left(GtkWidget *, gpointer data)
{
  ...
  //making sure user never zooms out past map bounds
  if(canvas->get_camera().get_world().left()-5<canvas->get_camera().get_initial_world().left()){
      return TRUE;
  }
...
gboolean press_right(GtkWidget *, gpointer data)
{
  ...
  //making sure user never zooms out past map bounds
  if(canvas->get_camera().get_world().right()+5>canvas->get_camera().get_initial_world().right()){
      return TRUE;
  }
